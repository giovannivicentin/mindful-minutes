  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {featuredPractices.map((practice) => (
        <Card key={practice.id} className="overflow-hidden border-none shadow-md">
          <div className={`h-3 bg-gradient-to-r ${practice.color}`} />
          <CardHeader>
            <div className="flex items-center gap-2">
              <div className="rounded-full bg-primary/10 p-2 text-primary">{practice.icon}</div>
          ...
          <CardFooter>
            <Link href={`/${locale}/practices/${practice.id}`} className="w-full">
