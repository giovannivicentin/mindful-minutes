...
import { Settings, X, Volume2, VolumeX, Maximize, Minimize } from "lucide-react"
...
export function TratakPractice({ locale, duration, isPaused = false }: TratakPracticeProps) {
  ...
  const [circleColor, setCircleColor] = useState(theme === "dark" ? "#FFFFFF" : "#000000")
  const [pointSize, setPointSize] = useState(8)
  const [pointColor, setPointColor] = useState(theme === "dark" ? "#FFFFFF" : "#000000")
  ...
  useEffect(() => {
    const checkAudioAvailability = async () => {
      try {
        ...
        const response = await fetch("/audio/meditation-bell.mp3", { method: "HEAD" })
        setAudioAvailable(response.ok)

        if (!response.ok) {
          console.warn("Audio files not available. Sound features will be disabled.")
        }
      } catch (error) {
  ...
  const toggleFullscreen = () => {
    ...
    if (!isFullscreen) {
      try {
        ...
        } else if ((tratakContainerRef.current as any).webkitRequestFullscreen) {
          /* Safari */
          ...
          {showControls && (
            ...
            >
              ...
              <Button
                ...
                className="bg-background/80 backdrop-blur-sm"
                aria-label={isFullscreen ? t("tratak.exitFullscreen") : t("tratak.fullscreen")}
              >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        {["#000000", "#FFFFFF", "#00B4B4", "#FFD700", "#FF5555", "#5555FF"].map((color) => (
                          <button
                            key={color}
                            className={`w-8 h-8 rounded-full ${
                              circleColor === color ? "ring-2 ring-offset-2 ring-primary" : ""
                      ...
                      <div className="flex gap-2">
                        {["#000000", "#FFFFFF", "#00B4B4", "#FFD700", "#FF5555", "#5555FF"].map((color) => (
                          <button
                            key={color}
                            className={`w-8 h-8 rounded-full ${
                              pointColor === color ? "ring-2 ring-offset-2 ring-primary" : ""
                            } ${color === "#FFFFFF" ? "border border-gray-200" : ""}`}
                            style={{ backgroundColor: color }}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        {["#FFFFFF", "#000000", "#F5F5F5", "#111111", "#001122", "#FFFFEE"].map((color) => (
                          <button
                            key={color}
                            className={`w-8 h-8 rounded-full ${
                              backgroundColor === color ? "ring-2 ring-offset-2 ring-primary" : ""
                            } ${
                              color === "#FFFFFF" || color === "#F5F5F5" || color === "#FFFFEE"
                    ...
                    <div className="flex items-center space-x-2">
                      <Switch id="pulsation" checked={showPulsation} onCheckedChange={setShowPulsation} />
                      <Label htmlFor="pulsation">{t("tratak.pulsation")}</Label>
                    </div>
                  </div>
                </PopoverContent>
              </Popover>
