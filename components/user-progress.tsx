...
export function UserProgress({ locale }: UserProgressProps) {
  ...
  useEffect(() => {
    ...
    const minutes = sessions.reduce((total, session) => total + session.duration, 0)
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">{t("profile.streak")}</CardTitle>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">{t("profile.totalTime")}</CardTitle>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">{t("profile.sessions")}</CardTitle>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">{t("profile.badges.title")}</CardTitle>
        ...
        <CardContent>
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className={`text-xs ${streak >= 7 ? "text-green-500" : "text-muted-foreground"}`}>
                {streak >= 7 ? "✓" : `${streak}/7`}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm">{t("profile.badges.hours10")}</span>
              <span className={`text-xs ${totalMinutes >= 600 ? "text-green-500" : "text-muted-foreground"}`}>
                {totalMinutes >= 600 ? "✓" : `${Math.floor(totalMinutes / 60)}/10`}
              </span>
            </div>
          </div>
        </CardContent>
